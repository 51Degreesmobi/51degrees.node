...
test('constructor arguments valid', function(t) {
  try {
    new Parser('../sampledata/51Degrees-Lite.1');
  } catch (e) {
...
test('pattern', function(t) {
  var parser = new Parser('../sampledata/51Degrees-Lite.dat', properties);
...
test('trie', function(t) {
  var parser = new Parser('../sampledata/51Degrees-Lite.trie', properties);
  var ret = parser.parse(userAgent);
  properties.forEach(function(property) {
    t.ok(typeof ret[property] !== undefined, property + '> ok');
  });
...
test('pattern overflow', function(t) {
  ...
  var parser = new Parser('../sampledata/51Degrees-Lite', properties);
  var throwed = false;
  try { parser.parse(ua); }
  catch (e) {
    throwed = true;
...
test('empty userAgent', function(t) {
  var r;
  var psr1 = new Parser('../sampledata/51Degrees-Lite', properties);
  r = psr1.parse('');
  t.equal(r.method, 'pattern');

  var psr2 = new Parser('../sampledata/51Degrees-Lite.trie', properties);
  r = psr2.parse('');
...
test('auto update', function(t) {
  ...
  var orgSrc = fs.readFileSync('../sampledata/51Degrees-Lite.dat');
  ...
  zlib.gzip(new Buffer('abcdefg', 'utf8'), function(err, data) {
    ...
    psr1 = new Parser('../sampledata/51Degrees-Lite', properties, {
      autoUpdate: true,
      key: 'license key',
      onupdated: onupdated
    });
  ...
  function onupdated(updated) {
    t.equal(fs.readFileSync('../sampledata/51Degrees-Lite.dat').length, 7);
    end();
  }

  function end() {
    fs.writeFileSync('../sampledata/51Degrees-Lite.dat', orgSrc);
    clock.restore();
    t.end();
  }
});
...
test('memory leak at trie', function(t) {
  var cp1 = process.memoryUsage().rss / 1024 / 1024;
  var psr1 = new Parser('../sampledata/51Degrees-Lite.trie', properties);
  ua_array.forEach(function(ua) {
    psr1.parse(ua);
  });
  var cp2 = process.memoryUsage().rss / 1024 / 1024;
